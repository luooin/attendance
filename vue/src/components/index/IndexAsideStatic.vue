  ...
  <el-aside class="index-aside" height="100vh" width="210px">
    <div class="index-aside-inner menulist" style="height:100%">
      <div v-for="item in menuList" :key="item.roleName" v-if="role == item.roleName" class="menulist-item"
        style="height:100%;broder:0;background-color:rgba(253,154,57,0.5)">
        <div class="menulistImg" style="backgroundColor:rgba(253,154,57,0.5);padding:30px 0"
        ...
        <el-menu mode="vertical" :unique-opened="true" class="el-menu-demo" style="height:100%;"
          background-color="rgba(253,154,57,0.5)" text-color="black" active-text-color="#FFFFFF" default-active="0">
          <el-menu-item index="(0).toString()" :style="menulistBorderBottom" @click="menuHandler('')"><i v-if="true"
              class="el-icon-s-home" />首页</el-menu-item>
          ...
          <el-submenu :style="menulistBorderBottom" v-for=" (menu, index) in item.backMenu" :key="menu.menu"
            :index="(index + 2).toString()">
            <template slot="title">
              <i v-if="true" :class="icons[index]" />
              <span>{{ menu.menu }}</span>
            </template>
            <el-menu-item v-for=" (child, sort) in menu.child" :key="sort" :index="((index + 2) + '-' + sort).toString()"
              @click="menuHandler(child.tableName)">{{ child.menu }}</el-menu-item>
          </el-submenu>
        </el-menu>
      ...
      this.$nextTick(() => {
        document.querySelectorAll('.menulist .el-menu-item').forEach(el => {
          el.addEventListener("mouseenter", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(255, 153, 52, 1)"
          })
          el.addEventListener("mouseleave", e => {
            e.stopPropagation()
            el.style.backgroundColor = "rgba(253,154,57,0.5)"
          })
    ...
    setMenulistIconColor() {
      this.$nextTick(() => {
        document.querySelectorAll('.menulist .el-submenu__title .el-submenu__icon-arrow').forEach(el => {
          el.style.color = "rgba(255, 153, 52, 1)"
        })
      })
    },
    menulistStyleChange() {
